...
TEST(Initialization, ZeroInitialization) {
    ...
    Aesi m7 = "Somebody once told me The world is gonna roll me I ain't the sharpest tool in the shed";
    EXPECT_EQ(m7, 0);
}
...
TEST(Initialization, BasicInitialization) {
    ...
    Aesi i02 = -1, i03 = 127, i04 = -127, i05 = -128, i06 = +127;
...
TEST(Initialization, SmallCharArrayInitialization) {
    Aesi ten = "10", negativeTen = "-10";
    EXPECT_EQ(ten, 10);
    EXPECT_EQ(negativeTen, -10);

    Aesi hexTenLC = "0xa", hexTenHC = "0xA", negativeHexTenLC = "-0xa", negativeHexTenHC = "-0xA";
...
TEST(Initialization, StringStringViewInitialization) {
    ...
    Aesi d0 = "489133282872437279"s;
    EXPECT_EQ(d0, 489133282872437279);
    Aesi d1 = "63018038201"sv;
    EXPECT_EQ(d1, 63018038201);
    Aesi d2 = "-489133282872437279"s;
    EXPECT_EQ(d2, -489133282872437279);
    Aesi d3 = "-63018038201"sv;
    EXPECT_EQ(d3, -63018038201);

    Aesi b0 = "0b11011001001110000000010000100010101011011101010101000011111"s;
    EXPECT_EQ(b0, 489133282872437279);
    Aesi b1 = "0b111010101100001010101111001110111001"sv;
    EXPECT_EQ(b1, 63018038201);
    Aesi b2 = "-0b11011001001110000000010000100010101011011101010101000011111"s;
    EXPECT_EQ(b2, -489133282872437279);
    Aesi b3 = "-0b111010101100001010101111001110111001"sv;
    EXPECT_EQ(b3, -63018038201);

    Aesi o0 = "0o106274176273174613"s;
    EXPECT_EQ(o0, 2475842268363147);
    Aesi o1 = "0o642054234601645202742"sv;
    EXPECT_EQ(o1, 7531577461358003682);
    Aesi o2 = "-0o106274176273174613"s;
    EXPECT_EQ(o2, -2475842268363147);
    Aesi o3 = "-0o642054234601645202742"sv;
    EXPECT_EQ(o3, -7531577461358003682);

    Aesi h0 = "0x688589CC0E9505E2"s;
    EXPECT_EQ(h0, 7531577461358003682);
    Aesi h1 = "0x3C9D4B9CB52FE"sv;
    EXPECT_EQ(h1, 1066340417491710);
    Aesi h2 = "-0x688589CC0E9505E2"s;
    EXPECT_EQ(h2, -7531577461358003682);
    Aesi h3 = "-0x3C9D4B9CB52FE"sv;
    EXPECT_EQ(h3, -1066340417491710);
    Aesi h4 = "0x688589cc0e9505e2"s;
    EXPECT_EQ(h4, 7531577461358003682);
    Aesi h5 = "0x3c9d4b9cb52fe"sv;
    EXPECT_EQ(h5, 1066340417491710);
    Aesi h6 = "-0x688589cc0e9505e2"s;
    EXPECT_EQ(h6, -7531577461358003682);
    Aesi h7 = "-0x3c9d4b9cb52fe"sv;
...
TEST(Initialization, Device) {
#ifdef __CUDACC__
    const auto kernel = [] __global__ (std::pair<Aesi, Aesi>& data) {
        const auto tid = threadIdx.x + blockIdx.x * blockDim.x;
        if(tid != 0) return;
        data.first = "123426017006182806728593424683999798008235734137469123231828679";
        data.second = 86833176188118864955;
    };

    std::pair values = { Aesi(), Aesi() };
    kernel<<<32, 32>>>(values);
    ...
    EXPECT_EQ(values.first, "123426017006182806728593424683999798008235734137469123231828679");
    EXPECT_EQ(values.second, 86833176188118864955);
#endif
}